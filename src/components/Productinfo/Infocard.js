...
function Infocard({ cardList, setCardList, card }) {
  ...
  const addOption = () => {
    let temp = [...addOptionList].concat('');
    setAddOptionList(temp);
  };

  const removeOption = (idx) => {
    let temp = [...addOptionList].filter((el, index) => index !== idx);
    setAddOptionList(temp);
  };
  return (
    <div className='info-card'>
      ...
      <div className='info-option'>
        <ul>
          <Optioninput option_name='제품명 / 중량' option_description='제품명/중량을 입력해 주세요' />
          <Optioninput option_name='원산지 / 원재료 함량' option_description='원산지 / 원재 함량을 입력해 주세요.' />
          <Optioninput option_name='등급' option_description='등급 (근내지방도 수치)를 입력해 주세요.' />
          <Optioninput option_name='보관' option_description='보관 방식을 입력해 주세요.' />
          {addOptionList.map((option, idx) => {
            return (
              <li className='option-item sub-option' key={idx}>
                <input type='text' className='sub-item-title' placeholder='항목 제목 자유 입력'></input>
